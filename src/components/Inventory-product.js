...
export function InventoryProduct (props) {
    ...
    async function updateQuanity () {
        ...
        if (inputQuantity >= 0 && inputQuantity <= 100 && inputQuantity !== '' ) {
            ...
            await axios.post('https://eden-backend.onrender.com/shop/update/quantity', data, {
                headers : { 
    ...
    async function deleteProduct () {
        AdminUserVerification()
        let response = await axios.post('https://eden-backend.onrender.com/shop/item/delete', {id : props.item._id}, {
            headers : {
                'authorization' : `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
