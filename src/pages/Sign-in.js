...
export function SignInPage () {
    ...
    useEffect(()=>{
        async function verifyUser () {
            let response = await axios.get('https://react-eden-backend.vercel.app/shop/getUser', {
    ...
    async function signIn () {
        ...
        let response = await axios.post('https://react-eden-backend.vercel.app/shop/loginUser', data, {
            headers : { 'Content-Type' : 'application/json' }
        }).then(res => res.data).catch(err => {return})

        if (response?.message === 'logged in') {
            localStorage.setItem('token', response?.token)
            let verify = await axios.get('https://react-eden-backend.vercel.app/shop/getUser', {
    ...
    async function signUp () {
        ...
        if (createPassword1 === createPassword2) {
            ...
            let response = await axios.post('https://react-eden-backend.vercel.app/shop/user', data, {
                headers : { 'Content-Type' : 'application/json' }
            }).then(res => res.data).catch(err => {return})
