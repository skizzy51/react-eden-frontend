...
export function AdminAddProduct () {
    ...
    async function addNormalProductTab () {
        ...
        let response = await axios.post('https://react-eden-backend.vercel.app/shop/normal-product-tab', data, {
            headers : {
    ...
    async function addSplitProductTab () {
        ...
        let response = await axios.post('https://react-eden-backend.vercel.app/shop/split-product-tab', data, {
            headers : {
                'authorization' : `Bearer ${token}`,
                'Content-Type' : 'application/json'
            }
    ...
    async function addNormalCategoryTab () {
        ...
        if (selectTag.current.value.length > 0) {
            let response = await axios.post('https://react-eden-backend.vercel.app/shop/normal-category-tab', {category : selectTag.current.id}, {
                headers : {
                    'authorization' : `Bearer ${token}`,
                    'Content-Type' : 'application/json'
                }
